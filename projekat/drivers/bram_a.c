...
static ssize_t bram_a_write(struct file *f, const char __user *buf, size_t length, loff_t *off)
{
  // Pisemo matricu A u BRAM memoriju
  char buff[BUFF_SIZE];
  int ret = 0;  
...
/*   
  if(ret != -EINVAL)//checking for parsing error
  {
    if (xpos > 255)
    {
      printk(KERN_WARNING "bram_a_write: X_axis position exceeded, maximum is 255 and minimum 0 \n");
    }
    else if (ypos > 143)
    {
      printk(KERN_WARNING "bram_a_write: Y_axis position exceeded, maximum is 143 and minimum 0 \n");
    }
    else
    {
  ...
  {
    printk(KERN_WARNING "bram_a_write: Wrong write format, expected \"xpos,ypos,rgb\"\n");
